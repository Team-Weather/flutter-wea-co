  ...
  group('SignUpUserUseCase 클래스', () {
    group('execute 메서드는', () {
      test('UserRepository.signUpUser()를 한번 호출한다.', () async {
        // Given
        UserAuth userAuth = UserAuth(email: 'test@email.com', password: 'password');
        ...
        // When
        final signUpUserUseCase = SignUpUserUseCase(userRepository: MockUserRepositoryImpl());
        await signUpUserUseCase.execute(userAuth, userProfile, userEmail);

        // Then
        expect(MockUserRepositoryImpl().signUp(userAuth, userProfile, userEmail), completion(true));
